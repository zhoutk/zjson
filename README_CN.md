...
2. **同一棵树的并发只读是安全的**：惰性 key 索引由 const 读路径以原子 CAS 发布（多线程竞争时共用一张已建好的表），成员键名在解析期即为 owned，所以 `operator[]`/`contains`/`findPtr`/`at`/迭代（含 `entry.key()`）都是纯读、不分配也不抛异常。**并发读写同一棵树仍不安全**（与 `std::string` 相同）；需要边读边写时请传副本或转移所有权。
